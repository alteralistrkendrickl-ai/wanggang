...
class Rotator(nn.Module):
    ...
    @staticmethod
    def to_spect(x, size=(256, 256)):
        shape = x.shape
        window_length = size[0]
        hop_length, zero_length = divmod(shape[-1] - size[0], size[1] - 1)
        if zero_length > 0:
            zero_length = size[1] - 1 - zero_length
            hop_length = hop_length + 1
            x = torch.nn.functional.pad(x, pad=(0, zero_length), mode='constant', value=0)
