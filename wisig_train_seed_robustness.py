"""Frozen additional-training-seed runs for the WiSig A1 study."""
...
SEEDS = (2025, 2026)
VARIANTS = ("B0", "A1S", "A1C")
CONFIRM_TOKEN = "RUN-WISIG-TRAIN-SEEDS-2025-2026"
COMMON_ARGS = (
    "--encoder", "CVTSLANet",
    "--TSLA_len", "256",
    "--TSLA_patch", "32",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Frozen WiSig training-seed robustness runs")
    ...
    parser.add_argument("--state-dir", default="runs/WiSig_train_seed_robustness_v1")
...
def build_command(project_root, protocol, variant, seed):
    return [
        sys.executable, str(project_root / "pretext.py"),
        "--dataset", f"wisig-{protocol}",
        *COMMON_ARGS,
    ...
    for seed in SEEDS:
        for variant in VARIANTS:
            ...
            tasks.append({
                ...
                "command": build_command(project_root, protocol, variant, seed),
            })
    return {
        "schema": "wisig-train-seed-robustness-v1",
        "protocol": protocol,
        "seeds": list(SEEDS),
...
def check_base_data(protocol):
    ...
    required = (
        root / "X_train_90Class.npy", root / "Y_train_90Class.npy",
        root / "X_val_90Class.npy", root / "Y_val_90Class.npy",
    )
...
def completed_task(task, recorded):
    output = Path(task["output_dir"])
    best, final = output / "best_encoder.pth", output / "final_encoder.pth"
    if not (best.is_file() and final.is_file()):
        return None
    current = {"best_sha256": sha256(best), "final_sha256": sha256(final)}
    if recorded is not None and recorded != current:
        raise RuntimeError(f"Completed checkpoint hash changed for {task['experiment_name']}")
    return current
...
def main():
    ...
    print("SEEDS=2025,2026")
    print("VARIANTS=B0,A1S,A1C")
    ...
    if args.mode == "audit":
        print("WISIG_TRAIN_SEED_AUDIT: PASS")
        return
    ...
    for task in manifest["tasks"]:
        ...
        recorded = status["tasks"].get(key, {}).get("checkpoints")
        complete = completed_task(task, recorded)
        if complete is not None:
            status["tasks"][key] = {"state": "complete", "checkpoints": complete}
        ...
        complete = completed_task(task, None)
        if complete is None:
    ...
    print("WISIG_TRAIN_SEED_ROBUSTNESS: PASS")
